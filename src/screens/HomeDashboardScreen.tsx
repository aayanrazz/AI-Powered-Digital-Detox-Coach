...
import { Alert, RefreshControl, Pressable, StyleSheet, Text, View } from 'react-native';
  ...
  return (
    ...
    >
      ...
      <View style={styles.row}>
        ...
        />
        <View style={{ width: 12 }} />
        <MetricCard label="Unread Alerts" value={dashboard?.unreadNotifications ?? 0} />
      </View>
      ...
      <View style={styles.panel}>
        ...
        {(dashboard?.aiRecommendations || []).length ? (
          dashboard?.aiRecommendations?.map((item, index) => (
            <Text key={`${item}-${index}`} style={styles.tip}>• {item}</Text>
          ))
        ) : (
          <Text style={styles.panelText}>
